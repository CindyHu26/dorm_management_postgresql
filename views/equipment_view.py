# views/equipment_view.py (v2 - 廠商關聯版)
                ...
                if submitted:
                    if not equipment_name:
                        ...
                        details = {
                            ...
                            "vendor_id": vendor_id, # 【核心修改 2】將 vendor_id 加入 details
    ...
    if equipment_df.empty:
        ...
        if selected_id:
                ...
                if details:
                    with st.form(f"edit_equipment_form_{selected_id}"):
                        ...
                        if edit_submitted:
                            update_data = { 
                                "dorm_id": e_dorm_id,
                                "vendor_id": e_vendor_id, # 【核心修改 4】將 vendor_id 加入更新資料
            ...
            with tab2:
                ...
                vendors_for_maintenance = vendor_model.get_vendors_for_view()
                vendor_options_for_maintenance = {v['id']: f"{v['服務項目']} - {v['廠商名稱']}" for _, v in vendors_for_maintenance.iterrows()} if not vendors_for_maintenance.empty else {}
                with st.form(f"quick_add_maintenance_{selected_id}", clear_on_submit=True):
                    ...
                    vendor_id = st.selectbox("執行廠商 (選填)", options=[None] + list(vendor_options_for_maintenance.keys()), format_func=lambda x: "未指定" if x is None else vendor_options_for_maintenance.get(x))
