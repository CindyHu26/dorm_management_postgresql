...
def get_loss_making_dorms(period: str):
    """
    【v1.2 支付方精準修正版】查詢在指定期間內，我司管理宿舍的詳細收支並篩選出虧損的項目。
    """
    ...
    try:
        if period == 'annual':
            ...
            query = f"""
                ...
                DormRent AS (
                    SELECT l.dorm_id, AVG(l.monthly_rent) as "月租金支出"
                    FROM "Leases" l JOIN "Dormitories" d ON l.dorm_id = d.id
                    WHERE d.rent_payer = '我司' AND l.lease_start_date <= CURRENT_DATE AND (l.lease_end_date IS NULL OR l.lease_end_date >= CURRENT_DATE)
                    GROUP BY l.dorm_id
                ),
                DormUtilities AS (
                    ...
                    WHERE ( (b.bill_type IN ('水費', '電費') AND d.utilities_payer = '我司') OR (b.bill_type NOT IN ('水費', '電費') AND b.payer = '我司') )
                      AND b.is_pass_through = FALSE AND TO_CHAR(b.bill_end_date, 'YYYY-MM') = '{period}'
                    GROUP BY b.dorm_id
                ),
                DormAmortized AS (
                ...
                LEFT JOIN DormUtilities du ON d.id = du.dorm_id LEFT JOIN DormAmortized da ON d.id = da.dorm_id
                WHERE d.primary_manager = '我司'
